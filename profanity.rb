...
Thread.new {
	begin
		...
		handle_game_text = proc { |text|
			...
			if current_stream.nil? or stream_handler[current_stream] or (current_stream =~ /^(?:death|logons|thoughts|voln|familiar|assess|atmospherics)$/)
				SETTINGS_LOCK.synchronize {
			...
			unless text.empty?
				if current_stream
					...
					elsif current_stream =~ /^(?:death|logons|thoughts|voln|familiar|assess|atmospherics)$/
						if window = stream_handler['main']
